...
import { getEmployeeSlots, createBooking, getLoyaltyStatus } from '../api/bookings';
...
const Booking = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      getLoyaltyStatus()
        .then((data) => {
          setNextBookingDiscounted(Boolean(data?.next_booking_discounted));
        })
        .catch((err) => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white py-8 dark:from-slate-900 dark:to-slate-950">
      <div className="mx-auto max-w-xl px-4">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-lg dark:bg-slate-800">
          ...
          {service && (
            <div className="mb-5 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-300">Price</p>
              {!nextBookingDiscounted ? (
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-200">
                  Full Price: <span className="font-semibold">${service.price.toFixed(2)}</span>
                </p>
              ) : (
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span className="text-sm text-slate-400 line-through dark:text-slate-500">${service.price.toFixed(2)}</span>
                  <span className="text-base font-semibold text-emerald-600 dark:text-emerald-400">${(service.price * 0.8).toFixed(2)}</span>
