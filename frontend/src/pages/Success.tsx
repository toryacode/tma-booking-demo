...
const Success = () => {
  ...
  const originalPrice = booking.original_price ?? booking.service?.price ?? 0;
  const finalPrice = booking.final_price ?? booking.service?.price ?? 0;
          ...
          <div className="mt-5 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
            ...
            {booking.discount_percent && booking.discount_percent > 0 ? (
              <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                <span className="text-slate-400 line-through dark:text-slate-500">${originalPrice.toFixed(2)}</span>
                <span className="font-semibold text-emerald-600 dark:text-emerald-400">${finalPrice.toFixed(2)}</span>
              ...
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                Full Price: <span className="font-semibold text-slate-800 dark:text-slate-100">${finalPrice.toFixed(2)}</span>
              </p>
            )}
          </div>
