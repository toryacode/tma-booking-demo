...
from app.schemas.booking import BookingCreate, Booking as BookingSchema, LoyaltyStatus
...
@router.get("/bookings/loyalty-status", response_model=LoyaltyStatus)
def get_loyalty_status(
    db: Session = Depends(get_db),
    user_id: str = Depends(get_current_user)
):
    reconcile_booking_statuses()
    return get_user_loyalty_status(db, user_id)
